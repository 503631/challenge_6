function showWinkelmandje() {
  ...
  if (document.webshop.winkelmandje_knop.checked) {
    ...
    document.getElementById("rijen").style.visibility = "hedden";
  }
  rijen =
    "<table><th width='120'>Titel</th><th>Artiest</th>" +
  ...
  for (x = 0; (x = lus); x++) {
    ...
    rij = "<tr>";
    titel = documet.getElementById("titel[" + x + "]").value;
    artiest = documet.getElementById("artiest[" + x + "]").value;
    genre = documet.getElementById("genre[" + x + "]").value;
    prijs = documet.getElementById("prijs[" + x + "]").value;
    aantal = documet.getElementById("aantal[" + x + "]").value;
    rij +=
      "<td width='120'>" +
      titel +
      "</td>" +
